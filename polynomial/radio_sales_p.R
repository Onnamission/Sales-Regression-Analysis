...
model1 = lm(sales ~ poly(radio, degree = 2, raw = TRUE), data = data_clean)

model2 = lm(sales ~ poly(radio, degree = 3, raw = TRUE), data = data_clean)
...
p1 = ggplot(data_clean, aes(x=radio, y=sales)) + 
...
p2 = ggplot(data_clean, aes(x=radio, y=sales)) + 
  geom_point(aes(color = radio, size = sales), alpha = 0.5) + 
